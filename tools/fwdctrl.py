...
def main():
    parser = argparse.ArgumentParser(prog="fwdctrl.py")

    parser.add_argument(
        "-c",
        "--control",
        metavar="127.0.0.1:8337",
        default="127.0.0.1:8337",
        help="Control server address",
    )
    ...
    socks_listen_addr.add_argument(
        "-l",
        "--listen",
        metavar="127.0.0.1:1080",
    ...
    remote_listen_addr.add_argument(
        "-l",
        "--listen",
        metavar="127.0.0.1:8445",
        help="Local connect address",
        required=True,
    )
    remote_addr = argparse.ArgumentParser(add_help=False)
    remote_addr.add_argument(
        "-r",
        "--remote",
        metavar="127.0.0.1:8445",
        help="Remote listen address",
        required=True,
    )
    r_add_parser = reverse_sub.add_parser(
        "add", parents=[remote_listen_addr, remote_addr]
    )
    r_add_parser.set_defaults(func=add_reverse)
    r_rm_parser = reverse_sub.add_parser(
        "rm", parents=[remote_listen_addr, remote_addr]
    )
    ...
    forward_remote_listen_addr.add_argument(
        "-l",
        "--listen",
        metavar="127.0.0.1:8445",
        help="Local connect address",
    ...
    forward_remote_addr.add_argument(
        "-r",
        "--remote",
        metavar="127.0.0.1:8445",
        help="Remote listen address",
    ...
    r_add_parser = forward_sub.add_parser(
        "add", parents=[forward_remote_listen_addr, forward_remote_addr]
    )
    r_add_parser.set_defaults(func=add_forward)
    r_rm_parser = forward_sub.add_parser(
        "rm", parents=[forward_remote_listen_addr, forward_remote_addr]
    )
